...
- 仅已白名单的只读工具进入首批模型列表。Server 的只读提示不是授权依据；未知/写入型 MCP 工具显示发现但禁用。报告写入走内置 `write_file` 及实际落盘回执，不泛化外部写入成功验证。按 2026-09-15 用户修订，模型可选择文件名并在一个 Run 中逐次新建多份，每次独立审批；输出路径不再是启用工具的前提，详见[工具方案 §7](../plans/2026-09-10-tool-runtime.md#7-write_file-的最小安全实现)。
