...
- 状态：**设计已核实，原文对照 Gate 的两项 P2 已关闭，定向设计 Gate：PASS；应用代码未开始修改。** 本文所有新增模块、表、接口、参数和限制均为拟议设计，不表示已实现。
- 本批假设：同一会话能在多次提问、服务重启和历史超过单次输入预算后继续使用，且历史、权限、工具结果不会串到其他会话。
- 当前需要补齐的能力：运行状态目前只在内存中，默认 Trace 也无法还原对话；需要持久化记录和受控的上下文构建。
- 设计完成条件：来源要求、数据归属、保存时机、恢复规则、长上下文策略和必过样例一致，并完成一次独立设计审阅。
- 实现完成条件：§11 固定验收通过。工具层已通过的结论继续有效；本设计不宣称会话模块已通过任何运行验收。
- 默认采用：SQLite 保存原始历史，最近完整轮次＋可追溯摘要组成 Context，需要时查询本会话原记录；支持文件任务及独立的历史对话追问。
- 本文只定义设计；[实施计划](../plans/2026-09-10-session-management.md)已另文编排，但应用、数据库和现有会话尚未修改，也未运行模型。
...
- 拟议默认目录：`~/.local/share/local-agent/`，数据库 `sessions.sqlite3`；`--state-dir` 仅由启动配置指定。此路径为新设计，尚未创建。
...
- 开启外键、SQLite WAL 与完整同步模式；短事务、有限 busy timeout（拟议 5 秒），不在模型/审批等待期间持数据库写事务。
...
- 摘要字段为目标、用户约束、已确认决定、完成事项、未完成事项及重要原文锚点。每条事实带 `message_id` 和原文范围；标题/代号/数字可直接保存原文，程序校验锚点归属及摘录一致性。摘要是导航信息，不是答案证据或权限。
- 摘要正文拟议上限 6 KiB，每次用户提交最多一次摘要模型请求，无工具；该请求计入本轮总模型调用上限 6 次、活动时限及 Usage，不能额外开无限摘要循环。只传需压缩的本会话数据。
...
- 搜索限本会话、当前 Run 开始前的已保存业务消息。首版字面关键词查询，不做向量库；返回最多 8 个命中、稳定 message ID、角色、时间、受限摘录和续页游标，使用参数绑定，不能把 query 当 SQL。
- read 按 message ID 返回原文，每页最多 8 KiB，明确 `complete/next_cursor`；分页按 Unicode 字符边界进行。工具协议和来源信息另占空间，结果信封总上限拟议 12 KiB。不可用的 ID、跨会话 ID 统一返回未找到。
...
ConversationPolicy 的拟议输出为 `status(answered/not_found/unable)`、`answer`、`references[{message_id,start,end}]`；摘录由程序从可见原文生成。answered 至少有一项合法历史来源；not_found 需要一次真实历史查询，文案限定本次查询范围；unable 需要真实查询或恢复故障。当前用户输入也可作为会话引用。该策略只用于会话入口，不放宽旧 `validate_answer`。
...
以下为设计验收标准，**尚未执行**。不能把工具层原来的 295 项通过计为会话模块已通过。
...
技术交接：本设计通过现有 Runtime 的 Context/journal 接口接入持久化；把业务存档与脱敏 Trace 分离，把历史来源与当前权限分离，把文件发布与 DB 提交之间的不确定性显式记录。会话功能的实现、离线验证、真实验证均未开始。

通俗说明：重启后能接着聊，是因为原话和做事记录留在本机；聊长了，模型先看近期对话和摘要，需要细节再回查。恢复记录不会自动重做文件动作，保存过的批准也不会变成永久通行证。

实施步骤已落到[会话管理实施计划](../plans/2026-09-10-session-management.md)。下一步从 Task 1 建立状态目录、单写者和 SQLite schema 开始；本文通过不表示代码或运行验收已经完成。
