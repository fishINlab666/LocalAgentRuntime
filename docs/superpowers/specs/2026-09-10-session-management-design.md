...
- 摘要字段为目标、用户约束、已确认决定、完成事项、未完成事项及重要原文锚点。摘要请求只为已验证的用户原话和有效助手回答提供由程序预先计算并签发 `span_id` 的 `reference_spans`；模型只返回按字段选择的 `span_id` 字符串，不能抄写或自行计算正文、消息编号与范围，也不能直接引用工具链。每个 ID 只选择一次并优先归入前五类最具体字段，`anchors` 只补充尚未覆盖的重要原文。程序先校验全部选择，再把已签发 ID 恢复为权威 `message_id/start/end/text` 并按实际 UTF-8 大小装箱：前五类核心状态必须整体保留，超限则本次摘要降级；`anchors` 跨字段去重后只在剩余预算内保留。未知 ID 或伪造旧范围同样使本次摘要降级。标题、代号和数字保存对应原文；摘要是导航信息，不是答案证据或权限。
