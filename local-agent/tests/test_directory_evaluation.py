...
class DirectoryEvaluationProvider:
    ...
    def complete(self, messages, tools, timeout):
        ...
        if len(results) == 1:
            return ModelReply(reads(*(entry['path'] for entry in results[0]['entries'])))
        ...
        if any(not item['ok'] for item in returned):
            ...
            for item in returned:
                lines = list(item['content'].values()) if isinstance(item['content'], dict) else item['content'].splitlines()
                answer += '\n'.join(lines) + '\n'
                citations.append({'path': item['path'], 'start_line': 1, 'end_line': len(lines)})
    ...
    def test_actual_read_error_can_stop_without_reading_the_other_file(self):
        class StopOnError(DirectoryEvaluationProvider):
            def complete(self, messages, tools, timeout):
                results = [json.loads(m['content']) for m in messages if m['role'] == 'tool']
                if len(results) == 1:
                    tiny = [entry['path'] for entry in results[0]['entries'] if entry.get('bytes', 100) < 20]
                    if tiny:
