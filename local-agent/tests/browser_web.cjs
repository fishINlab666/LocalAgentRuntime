...
(async () => {
  ...
  try {
    ...
    await page.locator('#trace summary').click();
    assert.match(await page.locator('#events').innerText(), /读取结果已回填/);
    ...
    console.log('PASS: directory mode, two-file citations, coverage, completed/active refresh, answers, HTML-as-text, not-found, missing-file, network error, cancel, late replies, path denial, 5 viewports, missing-key, no JS errors');
