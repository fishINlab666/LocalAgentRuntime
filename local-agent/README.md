...
输入问题并选择单文件或目录模式。模型自主发起 `read_file`，或先 `list_files` 发现路径再选文件读取；程序把带原调用编号的工具结果交回下一轮模型请求，再校验回答与本次原文引用。有写入权限的助手可自主提出文件名和完整内容，经逐次确认后新建多个文件，并将各自写入回执交回模型。输出路径可以留空。浏览器页面与命令行共用同一 Runtime。
...
页面的“助手与能力”区域可以安装、绑定 Skill/MCP 并创建新会话；详细字段、合成 MCP 示例和操作顺序见[助手与能力说明](examples/agents/README.md)。旧会话不会自动取得后来新增的能力。页面停用会取消其管理的受影响运行；CLI 停用 Skill 后尚不能可靠停止在途 Run，属于已知未关闭的 A2 验收缺口。密钥仍只从环境变量读取。
...
**工具层原切片及结果契约复核已通过；模型命名、多产物写入是 2026-09-15 的后续修订。** 写权限由助手配置决定，输出路径是可选的交付要求。Loop 会校验冻结参数、用户所见预览、工具错误及本次真实执行证明，写入成功还必须经过一次性发布并取得与批准内容一致的回执；结果始终按原调用 ID 进入下一轮。此前三个真实 run 的 10 次 DeepSeek 请求只证明原单产物切片，不能作为新增多产物真实验收。历史证据见[真实验证记录](trial/tool-layer-live-check.md)，当前状态以[阶段验收记录](trial/directory-check.md)为准，具体修订见[工具层方案](../docs/superpowers/plans/2026-09-10-tool-runtime.md)。后续开发遵循根目录[项目规则](../AGENTS.md)。
...
- 原内置范围：一个 DeepSeek Provider、`list_files`、`read_file` 及按助手权限注册的 `write_file`、一次任务的完整消息链、状态、取消/超时和本地日志；本阶段在相同链路增加 Skill/MCP 适配。
- 单文件模式只读用户指定的一份 UTF-8 `.md` / `.txt`；目录模式允许模型发现和选择最多 4 份文件，每份上限仍为 32 KiB。写工具可新建多个 `.md` / `.txt`，由模型提出工作区内相对路径，每份最多 32 KiB，分别确认并受 Run 预算限制；不覆盖、不自动创建父目录，没有 Shell。
...
```zsh
python3 -m local_agent run --workspace examples/discovery-workspace --discover --question '请核对项目代号、评审人与演示日期，引用原文并分别保存摘要和待办，自行决定文件名。'
```

程序逐份展示路径和完整内容，输入 `y` 确认或 `n` 拒绝。每次确认只批准当前文件，同一任务可创建多个不同路径。若必须生成特定文件，可加 `--output-file report.md`，该目标必须有实际回执才能判为完成。已有目标或缺少父目录会停止，由用户处理。网页和 CLI 的 `artifacts` 累加各文件的路径、字节数及 SHA-256；后续拒绝或回答失败不删除此前文件及回执。产物不作为本轮事实来源。
